...
import { organizationPathSchema, requestTodoHandoffBodySchema, requestTodoHandoffPathSchema, todoHandoffPathSchema } from '@amidala/contracts';
...
export function createTodoHandoffRoutes() {
  return new Hono<ApiEnv>()
    ...
    .post('/organizations/:organizationId/handoffs/:handoffId/accept', async (c) => { const path = todoHandoffPathSchema.safeParse(c.req.param()); if (!path.success) throw new ApiError('validation_error', 'Invalid path.'); const outcome = await (await c.get('scope').resolve<AcceptTodoHandoff>('acceptTodoHandoff')).execute(await currentUserId(c), path.data); return c.json({ handoff: outcome.handoff, todo: outcome.todo }); })
