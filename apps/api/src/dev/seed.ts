...
async function main() {
  ...
  try {
    ...
    for (const [email,name] of users) {
      // API package and @amidala/db resolve separate Drizzle instances; isolate the compatibility cast here.
    ...
    for (const [id,slug,name] of orgs) {
      ...
      const memberId = (email: string) => {
        const user = byEmail.get(email);
        if (!user) throw new Error(`Seed fixture incomplete: user ${email} is missing.`);
        return requireMembershipId(membershipRows, org.id, user.id, email);
      };
