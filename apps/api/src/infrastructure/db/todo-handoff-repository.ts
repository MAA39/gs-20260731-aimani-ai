...
export class TodoHandoffRepositoryDrizzle implements Port {
  ...
  private async decide(c: AcceptTodoHandoffCommand | RejectTodoHandoffCommand | CancelTodoHandoffCommand, verb: 'accepted' | 'rejected' | 'canceled') {
    ...
    }).catch((error) => { if (this.isRetryable(error)) throw new ApiError('service_unavailable', 'Database transaction could not be completed.'); throw error; });
  }
  ...
  async getTodoHandoffWorkspace(q: { organizationId: string; currentMembershipId: string }) {
    ...
    const rows = await this.db.select({ h: todoHandoff, t: todo }).from(todoHandoff).innerJoin(todo, and(eq(todo.id, todoHandoff.todoId), eq(todo.organizationId, todoHandoff.organizationId))).where(eq(todoHandoff.organizationId, q.organizationId)).orderBy(desc(todoHandoff.resolvedAt), desc(todoHandoff.id)).limit(20);
