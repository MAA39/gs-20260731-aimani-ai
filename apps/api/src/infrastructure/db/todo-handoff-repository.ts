...
export class TodoHandoffRepositoryDrizzle implements Port {
  ...
  async requestTodoHandoff(c: RequestTodoHandoffCommand): Promise<RequestTodoHandoffOutcome> {
    try {
      return await this.db.transaction(async (tx) => {
        ...
        const ids = [c.requesterMembershipId, c.recipientMembershipId].sort(); const lockedMembers = await tx.select().from(membership).where(inArray(membership.id, ids)).orderBy(membership.id).for('update'); const requester = lockedMembers.find((m) => m.id === c.requesterMembershipId); const recipient = lockedMembers.find((m) => m.id === c.recipientMembershipId); if (!requester || requester.organizationId !== c.organizationId) return { kind: 'forbidden' } as const;
    ...
    } catch (error) { if (this.isRetryable(error)) throw new Error('Retryable database transaction failure.'); if (this.isUniqueConflict(error)) { const [h] = await this.db.select().from(todoHandoff).where(and(eq(todoHandoff.todoId, c.todoId), eq(todoHandoff.organizationId, c.organizationId), eq(todoHandoff.status, 'requested'))).limit(1); if (h && h.requesterMembershipId === c.requesterMembershipId && h.recipientMembershipId === c.recipientMembershipId && h.requestMessage === c.requestMessage) { const s = await this.summary(this.db, h); return { kind: 'already_requested' as const, ...s }; } } throw error; }
  }
  ...
  private async decide(c: AcceptTodoHandoffCommand | RejectTodoHandoffCommand | CancelTodoHandoffCommand, verb: 'accepted' | 'rejected' | 'canceled') {
    ...
    return this.db.transaction(async (tx) => {
      ...
      const ids = [hint.requesterMembershipId, hint.recipientMembershipId].sort(); await tx.select().from(membership).where(inArray(membership.id, ids)).orderBy(membership.id).for('update');
  ...
  async getTodoHandoffWorkspace(q: { organizationId: string; currentMembershipId: string }) {
    ...
    const rows = await this.db.select({ h: todoHandoff, t: todo }).from(todoHandoff).innerJoin(todo, and(eq(todo.id, todoHandoff.todoId), eq(todo.organizationId, todoHandoff.organizationId))).where(eq(todoHandoff.organizationId, q.organizationId)).orderBy(desc(todoHandoff.requestedAt)).limit(100);
    const summaries = await Promise.all(rows.map((r) => this.summary(this.db, r.h, r.t))); return { organization: { organizationId: o?.organizationId ?? q.organizationId, name: o?.name ?? '' }, currentMember: member(q.currentMembershipId, cur?.displayName, cur?.title), incomingRequests: summaries.filter((x) => x.handoff.recipient.membershipId === q.currentMembershipId && x.handoff.status === 'requested').map((x) => x.handoff), outgoingRequests: summaries.filter((x) => x.handoff.requester.membershipId === q.currentMembershipId && x.handoff.status === 'requested').map((x) => x.handoff), recentHandoffs: summaries.filter((x) => x.handoff.status !== 'requested').map((x) => x.handoff) };
  }
  async getAssignedTodoWorkspace(q: { organizationId: string; currentMembershipId: string }) {
    const [o] = await this.db.select({ organizationId: organization.id, name: organization.name }).from(organization).where(eq(organization.id, q.organizationId)); const [cur] = await this.db.select().from(membership).where(and(eq(membership.id, q.currentMembershipId), eq(membership.organizationId, q.organizationId)));
    const creator = alias(membership, 'assigned_creator'); const assignee = alias(membership, 'assigned_assignee'); const rows = await this.db.select({ t: todo, creatorName: creator.displayName, creatorTitle: creator.title, assigneeName: assignee.displayName, assigneeTitle: assignee.title, h: todoHandoff }).from(todo).leftJoin(creator, and(eq(creator.id, todo.creatorMembershipId), eq(creator.organizationId, q.organizationId))).leftJoin(assignee, and(eq(assignee.id, todo.assigneeMembershipId), eq(assignee.organizationId, q.organizationId))).leftJoin(todoHandoff, and(eq(todoHandoff.todoId, todo.id), eq(todoHandoff.organizationId, todo.organizationId), eq(todoHandoff.status, 'requested'))).where(and(eq(todo.organizationId, q.organizationId), eq(todo.assigneeMembershipId, q.currentMembershipId)));
    return { organization: { organizationId: o?.organizationId ?? q.organizationId, name: o?.name ?? '' }, currentMember: member(q.currentMembershipId, cur?.displayName, cur?.title), todos: rows.map((r) => ({ todoId: r.t.id, organizationId: r.t.organizationId, contextMembershipId: r.t.contextMembershipId, title: r.t.title, description: r.t.description, status: r.t.status as 'open' | 'completed', creator: member(r.t.creatorMembershipId, r.creatorName, r.creatorTitle), assignee: member(r.t.assigneeMembershipId, r.assigneeName, r.assigneeTitle), createdAt: r.t.createdAt.toISOString(), updatedAt: r.t.updatedAt.toISOString(), pendingHandoff: r.h ? { handoffId: r.h.id, requester: member(r.h.requesterMembershipId, null, null), recipient: member(r.h.recipientMembershipId, null, null), requestMessage: r.h.requestMessage, requestedAt: r.h.requestedAt.toISOString() } : null })) };
  }
