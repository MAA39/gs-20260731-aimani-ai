...
export function RootRoute() {
  return <div className="app-shell">
    ...
    <nav className="bottom-nav" aria-label="モバイルナビゲーション">{links.map(({to,label,icon:Icon}) => <Link key={label} to={to} className="nav-link" activeProps={{className:'nav-link active'}}><Icon size={19} aria-hidden="true"/><span>{label}</span></Link>)}</nav>
  </div>;
}
