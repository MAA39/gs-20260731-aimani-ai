...
export function TodayPage({ organizationId, assignedResult, handoffResult, retry }: { organizationId: string; assignedResult: AssignedTodoWorkspaceResult; handoffResult: TodoHandoffWorkspaceResult; retry?: () => void }) {
  ...
  if (assignedResult.status !== 'ok' || handoffResult.status !== 'ok') {
    const error = assignedResult.status !== 'ok' ? assignedResult.error : handoffResult.error;
    return <section className="content"><div className="empty-surface"><h2>今日のボールを読み込めませんでした</h2><p>{error.message}</p>{retry ? <button className="secondary-button" type="button" onClick={retry}><RefreshCw size={16} aria-hidden="true" />再試行</button> : null}<Link className="secondary-button" to="/organizations">組織を選び直す</Link></div></section>;
  }
