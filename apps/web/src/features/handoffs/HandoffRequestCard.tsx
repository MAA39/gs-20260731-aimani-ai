...
export function HandoffRequestCard({ handoff, kind, currentMembershipId, onAnnounce }: { handoff: TodoHandoffSummary; kind: 'incoming' | 'outgoing' | 'recent'; currentMembershipId: string; onAnnounce: (message: string) => void }) {
  const mutation = useMutation({ mutationFn: async (action: 'reject' | 'cancel') => action === 'reject' ? rejectTodoHandoff({ data: { organizationId: handoff.organizationId, handoffId: handoff.handoffId } }) : cancelTodoHandoff({ data: { organizationId: handoff.organizationId, handoffId: handoff.handoffId } }) });
  ...
  async function decide(action: 'reject' | 'cancel') {
    ...
    try {
      ...
      const message = action === 'reject' ? '引き継ぎを見送りました。' : '引き継ぎ依頼を取り消しました。'; setResultMessage(message); onAnnounce(message);
    } catch { const message = '引き継ぎの処理に失敗しました。再試行してください。'; setResultMessage(message); onAnnounce(message); }
  }
  async function handleAccepted(result: TodoHandoffMutationResult) {
    if (result.status !== 'ok') return;
    await invalidate();
    const message = acceptedHandoffAnnouncement(result.handoff.recipient.name, result.handoff.nextAction);
    setResultMessage(message); onAnnounce(message);
  }
  ...
  const cardPending = mutation.isPending || acceptPending;
  return <article className={`handoff-card ${handoff.status}`} aria-busy={cardPending}><div className="handoff-card-copy"><div className="todo-card-heading"><h3>{handoff.todo.title}</h3><span className="handoff-status">{handoff.status === 'requested' ? '引き継ぎを依頼中' : handoff.status === 'accepted' ? '引き継ぎ済み' : handoff.status === 'rejected' ? '引き継ぎを見送り' : '引き継ぎ依頼を取消済み'}</span></div><div className={`handoff-rail rail-${handoff.status}`} aria-label={`${handoff.requester.name}から${handoff.recipient.name}への${railCopy}`}><span>{handoff.requester.name}</span><span className="todo-rail-line">{handoff.status === 'accepted' ? 'Todo →' : handoff.status === 'requested' ? 'Todo · 提案 →' : 'Todo · 変更なし'}</span><span>{handoff.recipient.name}</span></div>{handoff.requestMessage ? <p className="handoff-message">{handoff.requestMessage}</p> : null}{kind === 'recent' && handoff.nextAction ? <p className="handoff-next-action"><strong>次の一手</strong>{handoff.nextAction}</p> : null}{displayedAt && date ? <time dateTime={displayedAt}>{date}</time> : null}</div><div className="handoff-actions" aria-live="polite">{cardPending ? <span>処理中…</span> : null}{resultMessage ? <span role="status">{resultMessage}</span> : null}{kind === 'incoming' && !terminal ? <><AcceptTodoHandoffDialog handoff={handoff} disabled={cardPending} onPendingChange={setAcceptPending} onAccepted={handleAccepted} onFailure={(message) => { setResultMessage(message); onAnnounce(message); }} /><button className="secondary-button" disabled={cardPending} onClick={() => decide('reject')}>見送る</button></> : null}{kind === 'outgoing' && !terminal ? <button className="quiet-button" disabled={cardPending} onClick={() => decide('cancel')}>依頼を取り消す</button> : null}{canReviewAcceptedTodo ? <Link className="quiet-button" to="/$organizationId/todos" params={{ organizationId: handoff.organizationId }}>自分のTodoで確認</Link> : null}</div></article>;
}
