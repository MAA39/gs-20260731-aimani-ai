...
export function PeoplePage({ organization, organizations, result, retry }: { organization: OrganizationMembershipSummary; organizations: OrganizationMembershipSummary[]; result: GetPeopleResult; retry?: () => void }) {
  ...
  return <section className="content"><div className="people-context"><div><p className="eyebrow">{organization.name}</p><h2>関わる人</h2><p>誰と何を進めるかを、関係から選びます。</p></div><OrganizationSwitcher current={organization} organizations={organizations} /></div><div className="people-grid">{result.people.map((person) => <PersonCard key={person.membershipId} person={person} />)}</div></section>;
}
