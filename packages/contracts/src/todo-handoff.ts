...
export const acceptTodoHandoffBodySchema=z.object({nextAction:z.preprocess((value) => typeof value === 'string' ? value.trim() || undefined : value, z.string().min(1).max(240).optional())});
...
export const todoHandoffSummarySchema=z.object({handoffId:z.string(),organizationId:z.string(),todo:todoSummarySchema,requester:todoMemberSummarySchema,recipient:todoMemberSummarySchema,requestMessage:z.string().nullable(),nextAction:z.string().nullable(),status:todoHandoffStatusSchema,requestedAt:z.string(),resolvedAt:z.string().nullable()});
